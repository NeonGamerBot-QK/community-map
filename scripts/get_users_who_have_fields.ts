...
async function withRetry<T>(fn: () => Promise<T>, maxRetries = 10): Promise<T> {
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            if (error.data?.error === "ratelimited" || error.code === "rate_limited") {
                const retryAfter = error.data?.retry_after || Math.pow(2, i);
                console.log(`Rate limited. Retrying after ${retryAfter}s (attempt ${i + 1}/${maxRetries})`);
...
do {
    const res = await withRetry(() => web.users.list({
        limit: 200,
        cursor: cursor,
        include_locale: true,
    }));
    ...
    for (const member of res.members) {
        ...
        const profile = await withRetry(() => web.users.profile.get({
            user: member.id!,
            include_labels: true,
        })).then((d) => d.profile);
