...
do {
    ...
    for (const member of res.members) {
        ...
        if (profile && profile.fields) {
            ...
            if (location_field || school_field) {
                ...
                // write to file
                fs.writeFileSync(path.join(__dirname, `./users_with_fields.json`), JSON.stringify(users, null, 2));
    ...
    console.log(`Processed ${total_user_count} users, found ${users.length} users with fields so far.`);
}
while (cursor);

console.log(`Finished processing. Total users processed: ${total_user_count}. Users with fields: ${users.length}.`);