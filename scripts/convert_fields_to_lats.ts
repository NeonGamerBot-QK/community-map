...
const loadCache = (): Record<
  string,
  { lat: number; long: number; confidence: number }
> => {
  try {
...
const saveCache = (
  cache: Record<string, { lat: number; long: number; confidence: number }>,
) => {
...
const geocodeWithNominatim = async (
  location: string,
): Promise<{ lat: number; long: number; confidence: number } | null> => {
  if (
    !location ||
    location.toLowerCase().match(/no|nowhere|hq|chillin|cat kingdom/)
  ) {
    return null;
  }
...
const geocodeWithAI = async (
  users: User[],
  ai: OpenAI,
): Promise<
  Array<{ id: string; lat: number; long: number; confidence: number }>
> => {
  try {
      ...
      messages: [
        ...
        },
        {
          ...
          content: JSON.stringify(
            users.map((u) => ({ id: u.id, location: u.location_field })),
          ),
        },
      ],
    });
...
const main = async () => {
  const data: User[] = JSON.parse(
    fs.readFileSync(path.join(__dirname, "users_with_fields.json"), "utf8"),
  );
  ...
  console.log("\nSummary:");
  console.log(
    `- Geocoded: ${allResults.filter((r) => r.method === "geocoding").length}`,
  );
  console.log(`- AI: ${allResults.filter((r) => r.method === "ai").length}`);
  console.log(
    `- Failed: ${allResults.filter((r) => r.method === "failed").length}`,
  );
